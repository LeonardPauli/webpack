	...
	normalModuleFactory.plugin("factory", function(factory) {
		return function(data, callback) {
			...
			if (!data.dependencies) {
				console.warn('In webpack/lib/ExternalModuleFactoryPlugin, data.dependencies is now required' +
					(data.dependency ? ', the depricated dependency is used' : ''))
				dependency = data.dependency;
			} else dependency = data.dependencies[0];
